...
function ScoreNumber({ end }: { end: number }) {
  ...
  return (
    <span ref={ref} className="font-display text-6xl md:text-7xl font-black text-foreground tabular-nums">
      {value}
      <span className="text-accent">.{Math.round((end * 10) % 10)}</span>
    </span>
  );
}
    ...
    <section className="relative py-24 px-6 bg-background overflow-hidden">
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        <div ref={left.ref} className={left.className}>
          <div className="relative rounded-3xl border border-border bg-gradient-to-br from-white/[0.04] to-white/[0.01] p-8 backdrop-blur-sm">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="px-3 py-1 rounded-full bg-[#4CAF50]/10 border border-[#7EBF8E]/30 text-accent text-xs font-bold">
                Em evolução
              </span>
            </div>
              ...
              {[
                ...
                <div key={row.label}>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    ...
                    <span className="text-foreground font-bold tabular-nums">{row.v}%</span>
                  </div>
        ...
        <div ref={right.ref} className={right.className}>
          ...
          <ul className="space-y-3">
            {deliverables.map((d) => (
              <li key={d} className="flex items-start gap-3 text-sm text-foreground/65">
